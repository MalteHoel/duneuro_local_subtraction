...
namespace duneuro
{
  ...
  public: 
    ...
    DriverInterface(std::shared_ptr<VolumeConductorInterface<dim>> volumeConductor)
     ...
     */
    void write(const Dune::ParameterTree& config, DataTree dataTree = DataTree()) const
    {
    ...
    computeEEGTransferMatrix(const Dune::ParameterTree& config, DataTree dataTree = DataTree())
    {
     ...
     */
    std::unique_ptr<DenseMatrix<FieldType>>
    computeMEGTransferMatrix(const Dune::ParameterTree& config, DataTree dataTree = DataTree())
    {
    ...
    applyEEGTransfer(const DenseMatrix<FieldType>& transferMatrix,
                     ...
                     DataTree dataTree = DataTree())
    {
      return volumeConductor_->applyEEGTransfer(transferMatrix, dipole, config, dataTree);
    }
    ...
    applyMEGTransfer(const DenseMatrix<FieldType>& transferMatrix,
                     ...
                     DataTree dataTree = DataTree())
    {
      return volumeConductor_->applyMEGTransfer(transferMatrix, dipole, config, dataTree);
    }
