...
namespace duneuro
{
  ...
  public:
    ...
    virtual void assembleRightHandSide(VectorType& vector) const override
    {
      ...
      {
        if(isP1FEM<FS>::value && dim == 3) {
          /* note that even though we now at this point that dim == 3, the conditional below is necessary for compilation. The reason for this is that,
