...
namespace duneuro
{
  ...
  struct DenseMatrixToHDF5Reader<Dune::BCRSMatrix<Dune::FieldMatrix<T, blockSizeRow,
    ...
    read(const std::string& filename, const std::string& name = "matrix")
    {
      ...
      auto matrix = std::make_unique<MatrixType>(rows, cols, rows * cols, MatrixType::row_wise);
  ...
  struct DenseMatrixToHDF5Reader<Dune::DynamicMatrix<T>> {
                                                        ...
                                                        const std::string& name = "matrix")
    {
      ...
      auto matrix = std::make_unique<MatrixType>(dims[0], dims[1]);
  ...
  struct DenseMatrixToHDF5Reader<Eigen::Matrix<T, Rows, Cols, Eigen::ColMajor>> {
    ...
    read(const std::string& filename, const std::string& name = "matrix")
    {
      ...
      auto matrix = std::make_unique<MatrixType>(rows, cols);
      std::cout << "starting to read dense matrix from HDF" << std::endl;
  ...
  std::unique_ptr<M> readMatrixFromHDF5(const std::string& filename,
                                        const std::string& name = "matrix")
  {
    return DenseMatrixToHDF5Reader<M>::read(filename, name);
  }
#endif
  ...
  {
    auto fileFormat = config.get<std::string>("format");
    auto fileName = config.get<std::string>("filename");
    if (fileFormat == "hdf5") {
#if HAVE_HDF5WRAP
      return readMatrixFromHDF5<M>(fileName, config.get<std::string>("name", "matrix"));
#endif
    } else {
