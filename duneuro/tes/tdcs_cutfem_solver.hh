#ifndef DUNEURO_TDCS_CUTFEM_SOLVER_HH
#define DUNEURO_TDCS_CUTFEM_SOLVER_HH
...
namespace duneuro
{
  ...
  struct CutFEMTDCSSolverTraits {
    static const unsigned int dimension = ST::dim;
    using Solver = CutFEMSolver<ST, compartments, degree, TDCSPatchUDGParameter<typename ST::GridView>,
                             DF, RF, JF>;
  ...
  template <class ST, int compartments, int degree, class DF = double, class RF = double,
            class JF = double>
  class CutFEMTDCSSolver
  {
  public:
    using Traits = CutFEMTDCSSolverTraits<ST, compartments, degree, DF, RF, JF>;

    CutFEMTDCSSolver(std::shared_ptr<typename Traits::SubTriangulation> subTriangulation,
...
#endif // TDCS_DUNEURO_CUTFEM_SOLVER_HH
